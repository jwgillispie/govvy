...
class RepresentativeService {
  ...
  Representative _processMemberWithContactInfo(Map<String, dynamic> member) {
  String chamber = '';
  String state = '';
  String? district;
  String? office;
  String? phone;
  String? email;      // Added for email extraction
  String? website;
  String party = '';
  ...
  if (member.containsKey('terms') && member['terms'] != null) {
    ...
    if (terms is List && terms.isNotEmpty) {
      ...
      // Look for email or contact form in term
      if (term.containsKey('email')) {
        email = term['email']?.toString();
      } else if (term.containsKey('contact_form')) {
        email = term['contact_form']?.toString();
      }
    } else if (terms is Map && terms.containsKey('item')) {
      final items = terms['item'];
      if (items is List && items.isNotEmpty) {
  ...
  // Try to get address and contact information from various sources
  
  // 1. Check addressInformation
  if (member.containsKey('addressInformation') && member['addressInformation'] is Map) {
    final Map<String, dynamic> addressInfo = Map<String, dynamic>.from(member['addressInformation'] as Map);
    if (office == null) {
      office = addressInfo['officeAddress']?.toString();
    }
    ...
    if (email == null && addressInfo.containsKey('email')) {
      email = addressInfo['email']?.toString();
    }
  }
  
  // 2. Check contactInformation
  if (member.containsKey('contactInformation') && member['contactInformation'] is Map) {
    final Map<String, dynamic> contactInfo = Map<String, dynamic>.from(member['contactInformation'] as Map);
    ...
    if (website == null && contactInfo.containsKey('website')) {
      website = contactInfo['website']?.toString();
    }
  }
  
  // 3. Check generic contacts array if available
  if (member.containsKey('contacts') && member['contacts'] is List) {
    final contacts = member['contacts'] as List;
    
    for (var contactItem in contacts) {
      if (contactItem is Map) {
        final contact = Map<String, dynamic>.from(contactItem);
        ...
        if (value.isNotEmpty) {
          if (type == 'phone' && phone == null) {
            phone = value;
          } else if ((type == 'email' || type == 'mail') && email == null) {
            email = value;
          } else if (type == 'website' && website == null) {
            website = value;
          }
        }
      }
  ...
  // 4. Check direct email field if it exists
  if (email == null && member.containsKey('email')) {
    email = member['email']?.toString();
  }
  
  // 5. Check contact form field if email is still null
  if (email == null && member.containsKey('contactForm')) {
    email = member['contactForm']?.toString();
  } else if (email == null && member.containsKey('contact_form')) {
    email = member['contact_form']?.toString();
  }
  
  // Get official website URL if not found elsewhere
  if (website == null && member.containsKey('officialWebsiteUrl')) {
    website = member['officialWebsiteUrl']?.toString();
  }
  
  // Extract social media accounts if available
  if (member.containsKey('socialMedia') && member['socialMedia'] is List) {
    final socialMedia = member['socialMedia'] as List;
    
    for (var account in socialMedia) {
      if (account is Map) {
        final accountInfo = Map<String, dynamic>.from(account);
        if (accountInfo.containsKey('type') && accountInfo.containsKey('account')) {
          final type = accountInfo['type']?.toString() ?? '';
          final accountId = accountInfo['account']?.toString() ?? '';
          
          if (type.isNotEmpty && accountId.isNotEmpty) {
            socialMediaLinks.add('$type: $accountId');
          }
        }
      }
    }
