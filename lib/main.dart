...
Future<void> _loadEnvironmentVariables() async {
  try {
    ...
    if (congressApiKey == null) {
      ...
      debugPrint("CONGRESS_API_KEY loaded successfully (${congressApiKey.substring(0, min(3, congressApiKey.length))}...)");
    }
    ...
    if (googleMapsApiKey == null) {
      ...
      debugPrint("GOOGLE_MAPS_API_KEY loaded successfully (${googleMapsApiKey.substring(0, min(3, googleMapsApiKey.length))}...)");
    }
    
    debugPrint(".env file loaded with ${dotenv.env.length} variables");
  } catch (e) {
    debugPrint("WARNING: Error loading .env file: $e");
    debugPrint("WARNING: API features may not work correctly without environment variables.");
  }
}
